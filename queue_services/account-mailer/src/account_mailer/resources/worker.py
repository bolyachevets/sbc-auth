...
def handle_eft_available_notification(message_type, email_msg):
    ...
    email_dict = common_mailer.process(
        org_id, admin_emails, template_name, subject, logo_url=logo_url, context_url=context_url
    )
    process_email(email_dict)
...
def handle_account_confirmation_period_over(message_type, email_msg):
    ...
    logo_url = email_msg.get("logo_url")
    email_dict = common_mailer.process(
        org_id, admin_coordinator_emails, template_name, subject, nsf_fee=nsf_fee, logo_url=logo_url
    )
    process_email(email_dict)
...
def handle_pad_invoice_created(message_type, email_msg):
    ...
    logo_url = email_msg.get("logo_url")
    email_dict = common_mailer.process(
        org_id, admin_coordinator_emails, template_name, subject, logo_url=logo_url, **args
    )
    process_email(email_dict)
