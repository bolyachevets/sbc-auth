...
def test_lock_account_mailer_queue(app, session, client):
    ...
    with patch.object(notification_service, "send_email", return_value=None) as mock_send:
        helper_add_event_to_queue(
            client, message_type=QueueMessageTypes.NSF_LOCK_ACCOUNT.value, mail_details=mail_details
        )
        mock_send.assert_called
        assert mock_send.call_args.args[0].get("recipients") == "foo@bar.com"
        assert mock_send.call_args.args[0].get("content").get("subject") == SubjectType.NSF_LOCK_ACCOUNT_SUBJECT.value
        ...
        with patch.object(RestService, "post", return_value=response):
            ...
            mail_details = {
                ...
                "invoice": {
                    ...
                    "references": [{"invoiceNumber": "REG00001", "id": 2, "statusCode": "COMPLETED"}],
                    "receipts": [
                        {
                            "receiptAmount": 100.0,
                            "id": 2,
            ...
            helper_add_event_to_queue(
                client, message_type=QueueMessageTypes.NSF_UNLOCK_ACCOUNT.value, mail_details=mail_details
            )
            mock_send.assert_called
            assert mock_send.call_args.args[0].get("recipients") == "foo@bar.com"
            assert (
...
def test_account_conf_mailer_queue(app, session, client):
    ...
    with patch.object(notification_service, "send_email", return_value=None) as mock_send:
        ...
        helper_add_event_to_queue(
            client, message_type=QueueMessageTypes.CONFIRMATION_PERIOD_OVER.value, mail_details=mail_details
        )
...
def test_account_pad_invoice_mailer_queue(app, session, client):
    ...
    with patch.object(notification_service, "send_email", return_value=None) as mock_send:
        ...
        helper_add_event_to_queue(
            client, message_type=QueueMessageTypes.PAD_INVOICE_CREATED.value, mail_details=mail_details
        )
        ...
        assert mock_send.call_args.args[0].get("attachments") is None
        assert mock_send.call_args.args[0].get("content").get("body") is not None
        assert f'hours on {get_local_formatted_date(datetime.now(), "%m-%d-%Y")}' in mock_send.call_args.args[0].get(
            "content"
        ).get("body")
...
def test_account_team_modified(app, session, client):
    ...
    with patch.object(notification_service, "send_email", return_value=None) as mock_send:
        ...
        helper_add_event_to_queue(
            client, message_type=QueueMessageTypes.TEAM_MEMBER_INVITED.value, mail_details=mail_details
        )
...
def test_online_banking_emails(app, session, client):
    ...
    with patch.object(notification_service, "send_email", return_value=None) as mock_send:
        ...
        helper_add_event_to_queue(
            client, message_type=QueueMessageTypes.ONLINE_BANKING_UNDER_PAYMENT.value, mail_details=mail_details
        )
        ...
        helper_add_event_to_queue(
            client, message_type=QueueMessageTypes.ONLINE_BANKING_OVER_PAYMENT.value, mail_details=mail_details
        )
        ...
        helper_add_event_to_queue(
            client, message_type=QueueMessageTypes.ONLINE_BANKING_PAYMENT.value, mail_details=mail_details
        )
...
def test_pad_failed_emails(app, session, client):
    ...
    with patch.object(notification_service, "send_email", return_value=None) as mock_send:
        ...
        helper_add_event_to_queue(
            client, message_type=QueueMessageTypes.PAD_SETUP_FAILED.value, mail_details=mail_details
        )
...
def test_payment_pending_emails(app, session, client):
    ...
    with patch.object(notification_service, "send_email", return_value=None) as mock_send:
        mail_details = {"accountId": id, "cfsAccountId": "12345678", "transactionAmount": 20.00}
        helper_add_event_to_queue(
            client, message_type=QueueMessageTypes.PAYMENT_PENDING.value, mail_details=mail_details
        )
...
def test_ejv_failure_emails(app, session, client):
    ...
    with patch.object(notification_service, "send_email", return_value=None) as mock_send:
        ...
        # add an event to queue
        mail_details = {"fileName": minio_file_name, "minioLocation": minio_bucket}
        helper_add_event_to_queue(client, message_type=QueueMessageTypes.EJV_FAILED.value, mail_details=mail_details)
...
def test_passcode_reset_email(app, session, client):
    ...
    with patch.object(notification_service, "send_email", return_value=None) as mock_send:
        ...
        helper_add_event_to_queue(client, message_type=QueueMessageTypes.RESET_PASSCODE.value, mail_details=msg_payload)
...
def test_statement_notification_email(app, session, client):
    ...
    with patch.object(notification_service, "send_email", return_value=None) as mock_send:
        ...
        helper_add_event_to_queue(
            client, message_type=QueueMessageTypes.STATEMENT_NOTIFICATION.value, mail_details=msg_payload
        )
...
def test_payment_reminder_notification_email(app, session, client):
    ...
    with patch.object(notification_service, "send_email", return_value=None) as mock_send:
        ...
        helper_add_event_to_queue(
            client, message_type=QueueMessageTypes.PAYMENT_REMINDER_NOTIFICATION.value, mail_details=msg_payload
        )
...
def test_payment_due_notification_email(app, session, client):
    ...
    with patch.object(notification_service, "send_email", return_value=None) as mock_send:
        ...
        helper_add_event_to_queue(
            client, message_type=QueueMessageTypes.PAYMENT_DUE_NOTIFICATION.value, mail_details=msg_payload
        )
